...
def coverage_rows() -> list[dict[str, Any]]:
    ...
    rows = [
        ...
        _row("coinalyze", tag=True, pair=True, value="Cross-exchange OI, funding, liquidations, long/short and OHLCV", api=True, free=True, card=False, trial=False, quota="40 API calls/minute/key; runtime schedule uses four calls per 15-minute collection", history=True, storage=True, role="derivatives-candidate", account=True, adapter="runtime_shadow_ready_not_deployed", influence=False, decision="scheduled runtime collector, freshness gate and owner-visible app evidence passed bounded exact TAG/USDT OI/funding reads; missing per-market values and liquidation rows remain unavailable, no heatmap is inferred, and Render activation remains pending", terms="https://api.coinalyze.net/v1/doc/", evidence=["Official Coinalyze API documentation defines the authenticated catalog, OI, funding and liquidation endpoints and a 40-call/minute/key limit.", "A bounded 2026-08-21 live runtime read found exact TAG/USDT perpetuals; missing per-market values remain unavailable and header authentication keeps the credential out of URLs and evidence."]),
