...
def coverage_rows() -> list[dict[str, Any]]:
    ...
    rows = [
        ...
        _row("nodereal", tag=True, pair=False, value="Archive BSC RPC, including historical logs", api=True, free=True, card=False, trial=False, quota="Free plan: 3 keys and documented monthly CU allowance; official pages currently conflict between 10M and 100M CU", history=True, storage=True, role="primary-onchain-candidate", account=True, adapter="generic_rpc_waiting_for_credentials", influence=False, decision="accepted as the highest-value credential candidate; verification is queued", terms="https://docs.nodereal.io/docs/pricing-plan", evidence=["Official FAQ says no card and archive data; exact quota conflict is preserved rather than resolved by guesswork."]),
        _row("moralis", tag=None, pair=False, value="Indexed BSC token, transfer and wallet history", api=True, free=True, card=None, trial=False, quota="Free throughput documented at 40 requests/second; monthly CU not revalidated", history=True, storage=None, role="optional-onchain", account=True, adapter="not_implemented", influence=False, decision="deferred: generic chain value exists but exact TAG response and storage rights are unverified", terms="https://moralis.com/pricing/", evidence=["https://docs.moralis.com/data-api/resources/rate-limits"]),
        _row("coinalyze", tag=None, pair=None, value="Cross-exchange OI, funding, liquidations, long/short and OHLCV", api=True, free=True, card=False, trial=False, quota="40 API calls/minute/key; intraday retains about 1,500-2,000 points; daily retained", history=True, storage=True, role="derivatives-candidate", account=True, adapter="waiting_for_credentials_and_market_list", influence=False, decision="deferred: free API is legitimate, but exact TAGUSDT support must be proven after key issuance", terms="https://api.coinalyze.net/v1/doc/", evidence=["Official API docs; no TAG support claim without authenticated supported-market response."]),
