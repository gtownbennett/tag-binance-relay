...
DEFAULT_PROVIDERS: tuple[Provider, ...] = (
    ...
    Provider("binance_vision", "Binance Vision", "primary-history", "immutable-public-archives", True, "configured", True),
    ...
    Provider("kucoin", "KuCoin", "corroborating", "spot+derivatives", True, "configured", True),
    Provider("okx", "OKX", "optional-venue", "market+derivatives", True, "rejected_no_verified_tag_instrument", False),
    Provider("bybit", "Bybit", "optional-venue", "market+derivatives", True, "inaccessible_unverified", False),
    Provider("dexscreener", "DEX Screener", "corroborating-dex", "pool+liquidity", True, "configured", True),
    ...
    Provider("publicnode", "PublicNode", "on-chain-rpc", "bounded-logs+read-only-rpc", True, "configured_shadow", False, "Public fallback used for bounded eth_getLogs; never forecast-weighted."),
    Provider("onerpc", "1RPC", "on-chain-rpc", "privacy-rpc", True, "quota_exhausted", False),
    Provider("bscscan", "BscScan", "on-chain-adapter", "labels+historical lookup", False, "rejected_chain_not_free", False, "Etherscan V2 free API access is not supported for BNB Smart Chain."),
    Provider("nodereal", "NodeReal MegaNode", "on-chain-adapter", "archive-bsc-rpc", True, "runtime_shadow_ready_not_deployed", False, "The scheduled read-only runtime collector and freshness API passed live BNB Chain and exact TAG-contract reads with the local DPAPI credential; Render activation remains pending."),
    Provider("moralis", "Moralis", "on-chain-adapter", "indexed-token+wallet-history", True, "blocked_exact_contract_response_unverified", False, "BNB Chain and generic ERC-20 support are documented, but no exact TAG contract response was authenticated."),
    Provider("coinalyze", "Coinalyze", "derivatives-adapter", "oi+funding+liquidations+long-short", True, "runtime_shadow_ready_not_deployed", False, "The scheduled read-only runtime collector and freshness API passed exact TAG/USDT OI/funding reads with the local DPAPI credential; missing liquidation rows remain missing and Render activation is pending."),
    Provider("dune", "Dune", "on-chain-research", "queryable-bsc-history", True, "rejected_overage_risk", False),
    Provider("goldrush", "GoldRush / Covalent", "on-chain-adapter", "decoded-wallet+token-history", True, "rejected_trial_only", False),
    Provider("bitquery", "Bitquery", "on-chain-adapter", "graphql+websocket-chain-data", True, "rejected_trial_only", False),
    Provider("cmc", "CoinMarketCap", "reference", "supply+market", True, "configured", True),
    Provider("coingecko", "CoinGecko", "reference", "identity+supply+market", True, "configured", True),
    Provider("coinranking", "Coinranking", "reference-candidate", "aggregate-market+history", True, "evaluated_not_integrated", False),
    Provider("coinpaprika", "CoinPaprika", "reference-candidate", "aggregate-market+history", True, "evaluated_not_integrated", False),
    Provider("coinlore", "CoinLore", "reference-candidate", "aggregate-market+history", True, "evaluated_not_integrated", False),
    Provider("defillama", "DefiLlama", "corroborating-candidate", "defi+protocol-metrics", True, "evaluated_not_integrated", False),
    Provider("firecrawl", "Firecrawl", "discovery-candidate", "rendered-page-discovery", True, "waiting_for_credentials", False),
    Provider("tavily", "Tavily", "discovery-candidate", "search+extract+map", True, "waiting_for_credentials", False),
    Provider("exa", "Exa", "discovery-candidate", "semantic-web-search", True, "waiting_for_credentials", False),
    Provider("hyblock", "Hyblock", "specialist", "liquidation+orderflow-analytics", False, "rejected_no_verified_tag_coverage", False),
    Provider("coinglass", "CoinGlass", "specialist", "liquidation+oi+funding-analytics", False, "rejected_paid_api", False),
    Provider("nansen", "Nansen", "specialist", "wallet-labels+smart-money", True, "evaluated_not_integrated", False),
    Provider("arkham", "Arkham", "specialist", "entity-labels+wallet-intelligence", False, "rejected_no_verified_tag_coverage", False),
    Provider("bubblemaps", "Bubblemaps", "specialist", "holder-cluster-visualization", False, "rejected_no_verified_api", False),
    Provider("coinank", "CoinAnk", "specialist", "derivatives+liquidation-analytics", False, "rejected_no_verified_tag_coverage", False),
    Provider("velo", "Velo", "specialist", "derivatives-timeseries", False, "rejected_paid_api", False),
    Provider("laevitas", "Laevitas", "specialist", "derivatives+volatility", False, "rejected_paid_api", False),
    Provider("cryptoquant", "CryptoQuant", "specialist", "on-chain+exchange-flows", False, "rejected_paid_api", False),
    Provider("kaiko", "Kaiko", "specialist", "institutional-market-data", False, "rejected_paid_api", False),
    Provider("amberdata", "Amberdata", "specialist", "institutional-onchain+market-data", False, "rejected_no_verified_free_api", False),
    Provider("glassnode", "Glassnode", "specialist", "on-chain-metrics", False, "rejected_no_verified_tag_coverage", False),
    Provider("santiment", "Santiment", "specialist", "social+on-chain+development-metrics", True, "evaluated_not_integrated", False),
    Provider("lunarcrush", "LunarCrush", "specialist", "social-intelligence", False, "rejected_no_verified_tag_coverage", False),
    Provider("token_terminal", "Token Terminal", "specialist", "protocol-fundamentals", False, "rejected_no_verified_tag_coverage", False),
    Provider("tokenomist", "Tokenomist", "specialist", "unlock-schedules", False, "rejected_no_verified_tag_coverage", False),
    Provider("tradingview", "TradingView", "specialist", "charts+technical-analysis", False, "rejected_no_supported_data_api", False),
    Provider("dextools", "DEXTools", "specialist", "dex-market-analytics", False, "rejected_no_verified_free_api", False),
    Provider("coinmonkey", "Coin Monkey / CoinMonkey", "specialist", "unresolved-provider-identity", False, "rejected_identity_unresolved", False),
...
def forecast_snapshot_fingerprint(snapshot: Mapping[str, Any]) -> str:
    ...
    stable = {
        ...
        "targetCurrency": snapshot.get("targetCurrency") or "USD",
